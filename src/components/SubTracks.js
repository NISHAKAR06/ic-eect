  ...
  return (
    <section id="sub-tracks" className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold text-center text-green-800 mb-12">SUB TRACKS</h2>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tracks.map((track, index) => (
            <div key={index} className="bg-white p-6 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
              <div className="text-5xl text-green-800 mb-4 text-center">{track.icon}</div>
              <h3 className="text-xl font-semibold text-green-800 mb-3 text-center">{track.title}</h3>
              <p className="text-gray-700 leading-relaxed text-sm">{track.description}</p>
            </div>
          ))}
        </div>

        <div className="mt-16 bg-green-600 text-white p-8 rounded-lg shadow-lg text-center max-w-3xl mx-auto">
